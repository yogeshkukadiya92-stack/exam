      ...
      <header className="sticky top-0 z-50 border-b border-slate-200/80 bg-white/80 backdrop-blur-xl dark:border-slate-700/80 dark:bg-slate-900/80">
        <div className="relative mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-3 sm:px-6">
            ...
            <nav className="hidden items-center gap-1 lg:flex">
              {navItems.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  className="flex items-center gap-1.5 rounded-lg px-2.5 py-2 text-sm font-medium text-slate-600 transition-colors hover:bg-slate-100 hover:text-slate-900 dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-slate-200 xl:px-3"
                >
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <ThemeToggle />
            <MobileNav items={navItems.map((i) => ({ href: i.href, label: i.label }))} className="lg:hidden" />
            <div className="hidden min-w-0 md:block">
              <p className="max-w-36 truncate text-sm font-medium text-slate-700 dark:text-slate-300 xl:max-w-52">
